...
# ==============================================================================
# DISCORD BOT ROLEPLAY (DISCORD.PY V2+)
# Sistema completo con Slash Commands (/), Form Modals e Database JSON
# Comandi inclusi: /cittadinanza, /patente, /porto_darmi, /documenti
# ==============================================================================
...
def get_bot_token():
    ...
    # Cerca se esiste già un file config.json
    ...
    # Chiede il token all'utente tramite console se manca
    print("=" * 60)
    print("🤖 CONFIGURAZIONE INIZIALE DISCORD BOT")
    print("=" * 60)
    ...
    # Salva il token in config.json per le prossime volte
    try:
        with open(config_file, "w", encoding="utf-8") as f:
            json.dump({"token": token}, f, indent=4)
        print("💾 Token salvato in 'config.json' per i prossimi avvii!")
    except Exception as e:
        print(f"⚠️ Impossibile salvare config.json: {e}")
        
    return token

# --- IMPOSTAZIONE INTENTS & BOT ---
...
# ==============================================================================
# FORM MODAL INTERATTIVI (DISCORD UI)
# ==============================================================================
...
class CittadinanzaModal(discord.ui.Modal, title="Richiesta Cittadinanza"):
    ...
    data_nascita = discord.ui.TextInput(
        ...
        placeholder="es. 15/05/1990",
        required=True,
        max_length=10
    )
    luogo_nascita = discord.ui.TextInput(
        label="Luogo di Nascita",
        placeholder="es. Los Santos",
        required=True,
        max_length=50
    )
    sesso = discord.ui.TextInput(
        label="Sesso (M/F/Altro)",
        placeholder="M o F",
        required=True,
        max_length=10
    )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(
            title="🪪 CARTA DI CITTADINANZA EMESSA",
        ...
        embed.add_field(name="Data Emissione", value=f"```\n{db[user_id]['cittadinanza']['data_emissione']}\n```", inline=True)
        embed.set_footer(text="Repubblica Roleplay • Documento Ufficiale")

        await interaction.response.send_message(content="✅ **Cittadinanza registrata con successo!**", embed=embed, ephemeral=False)


class PatenteModal(discord.ui.Modal, title="Emissione Patente di Guida"):
    categorie = discord.ui.TextInput(
        label="Categorie Abilitate",
        placeholder="es. A, B, C (oppure Auto, Moto, Camion)",
        default="A, B",
        required=True,
        max_length=50
    )
    punti = discord.ui.TextInput(
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if user_id not in db or "cittadinanza" not in db[user_id]:
            await interaction.response.send_message("⚠️ Non hai ancora registrato la Cittadinanza! Usa prima `/cittadinanza`.", ephemeral=True)
            return
        ...
        nome = db[user_id]["cittadinanza"]["nome_cognome"]

        embed = discord.Embed(
            title="🚗 PATENTE DI GUIDA EMESSA",
        ...
        embed.add_field(name="Stato", value="```yaml\nVALIDA\n```", inline=True)
        embed.set_footer(text="Dipartimento Trasporti & Motorizzazione")
...
class PortoDarmiModal(discord.ui.Modal, title="Richiesta Porto d'Armi"):
    tipo_licenza = discord.ui.TextInput(
        label="Tipo Licenza",
        placeholder="es. Difesa Personale, Caccia, Sportivo",
        default="Difesa Personale",
        required=True,
        max_length=50
    )
    armi_consentite = discord.ui.TextInput(
        label="Armi Consentite",
        placeholder="es. Pistole calibro 9mm, Fucili da caccia",
        required=True,
        max_length=100
    )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        embed.add_field(name="Stato Licenza", value="```yaml\nATTIVO\n```", inline=True)
        ...
        await interaction.response.send_message(content="✅ **Porto d'armi emesso con successo!**", embed=embed)

# ==============================================================================
# EVENTI BOT E COMANDI SLASH (/COMMANDS)
# ==============================================================================
...
@bot.tree.command(name="cittadinanza", description="Richiedi o crea la tua Carta di Cittadinanza IC.")
async def cittadinanza(interaction: discord.Interaction):
    await interaction.response.send_modal(CittadinanzaModal())

@bot.tree.command(name="patente", description="Richiedi o registra la tua Patente di Guida.")
async def patente(interaction: discord.Interaction):
    await interaction.response.send_modal(PatenteModal())

@bot.tree.command(name="porto_darmi", description="Richiedi il Porto d'Armi ufficiale.")
async def porto_darmi(interaction: discord.Interaction):
    await interaction.response.send_modal(PortoDarmiModal())

@bot.tree.command(name="documenti", description="Mostra tutti i tuoi documenti registrati o quelli di un altro utente.")
@app_commands.describe(utente="L'utente di cui vuoi visualizzare i documenti (Opzionale)")
async def documenti(interaction: discord.Interaction, utente: discord.User = None):
    ...
    if user_id not in db or not db[user_id]:
        await interaction.response.send_message(f"❌ Nessun documento trovato per **{target.display_name}**.", ephemeral=True)
        return

    user_docs = db[user_id]
    embed = discord.Embed(
        title=f"🗂️ Portafoglio Documenti - {target.display_name}",
    ...
    if "porto_darmi" in user_docs:
        ...
        embed.add_field(
            name="🔫 Porto d'Armi",
            value=f"**Tipo:** {pa['tipo']}\n**Armi:** {pa['armi']}\n**Stato:** {pa['stato']}",
            inline=False
        )

    embed.set_footer(text="Sistema Documentale Ufficiale Roleplay")
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("❌ Token non valido! Elimina 'config.json' e riavvia il bot per inserire il token corretto.")
